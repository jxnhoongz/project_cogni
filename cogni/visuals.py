...
def _build_prompt(scenes: list[dict[str, Any]], title: str, thesis: str) -> str:
    ...
    return (
        ...
        f"composition that fits the narration. When a person appears it is the recurring "
        f"protagonist as a stylized, faceted LOW-POLY figure — a clear low-poly face is good, "
        f"never photorealistic; otherwise use objects, settings, or symbolic imagery. Do NOT "
        f"mention art style (added later).\n"
        ...
        f"new or unrelated image. Same constraints (recurring protagonist only, stylized "
        f"low-poly, no style token).\n"
        ...
        f"motion. Calm and slow, never frantic, no NEW characters. BANNED phrasing "
